...
var imagePuzzle = {
    ...
    startGame: function (images,imagesActual, gridSize) {
        this.setImage(images,imagesActual, gridSize);
    ...
    tick: function () {
        ...
        var elapsedTime = 60-parseInt((now - imagePuzzle.startTime) / 1000, 10);
        helper.doc('timerPanel').textContent = elapsedTime;
        if(elapsedTime>=0)
        timerFunction = setTimeout(imagePuzzle.tick, 1000);
        else{
           if( helper.doc('gameOver').style.display !='block') alert("Session Time Out!... Refresh the page...");
           // helper.shuffle('sortable');
    ...
    setImage: function (images, imagesActual, gridSize = 4) {
        ...
        for (var i = 0; i < gridSize * gridSize; i++) {
            ...
            if(helper.doc('timerPanel').textContent<0){
